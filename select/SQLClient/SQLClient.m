...
int const SQLClientConnectTimeout = 10;
int const SQLClientQueryTimeout = 1800;
...
- (id)init
{
    if (self = [super init])
    {
		...
		self.timeout = SQLClientQueryTimeout;
	 ...
	 completion:(void (^)(BOOL success))completion
{
	...
	[self.workerQueue addOperationWithBlock:^{
		...
		//Set login timeout
		dbsetlogintime(SQLClientConnectTimeout);
    ...
    if (self.workerQueue.operationCount > 1) {
        [self.workerQueue.operations.lastObject addDependency:self.workerQueue.operations[self.workerQueue.operationCount - 2]];
    }
}

- (BOOL)connectWithinThread:(NSString*)host
       username:(NSString*)username
       password:(NSString*)password
       database:(NSString*)database
{
...
- (void)execute:(NSString*)sql
     curQueryID:(NSUInteger)curQueryID
      conParams:(Connection*)conParams
     completion:(void (^)(NSArray* results))completion
{
	...
	[self.workerQueue addOperationWithBlock:^{
        ...
        queryID = curQueryID;
        if (self.connected) {
            if (![self.host isEqualToString:conParams.server]
                || ![self.database isEqualToString:conParams.database]
                || ![self.username isEqualToString:conParams.login])
            {
                dbclose(connection);
                needToConnect = TRUE;
            }
        ...
        if (needToConnect) {
            if (![self connectWithinThread:conParams.server username:conParams.login password:conParams.password database:conParams.database])
            ...
            dbcmd(connection, [options UTF8String]);
...
int err_handler(DBPROCESS* dbproc, int severity, int dberr, int oserr, char* dberrstr, char* oserrstr)
{
	//Can't call self from a C function, so need to access singleton
	SQLClient* self = [SQLClient sharedInstance];
	[self error:[NSString stringWithUTF8String:dberrstr] code:dberr severity:severity queryID:queryID];
	return INT_CANCEL;
}
...
- (void)error:(NSString*)error code:(int)code severity:(int)severity queryID:(NSUInteger)queryID
{
	if (!self.delegate || ![self.delegate conformsToProtocol:@protocol(SQLClientDelegate)])
        [NSException raise:SQLClientDelegateError format:@"SQL server error"];
	
	//Invoke delegate on callback queue
	[self.callbackQueue addOperationWithBlock:^{
		[self.delegate error:error code:code severity:severity queryID:queryID];
	}];
}
